...
- Регулярные добровольные пожертвования технически готовятся только как ежемесячные автоплатежи YooKassa и включаются runtime-переменной после отдельного подключения автоплатежей YooKassa.
- Для регулярных пожертвований использовать self-service управление по защищенной ссылке из email, без публичной регистрации WordPress-пользователей; жертвователь должен иметь возможность самостоятельно отключить ежемесячное пожертвование и удалить сохраненный способ оплаты из системы сайта.
